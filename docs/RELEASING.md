...
4. 检查并记录 Windows 代码签名状态。本项目当前发布 `v0.1.4` 未签名 prerelease，Release
...
```powershell
$installer = 'release\Grok-Desktop-Windows-x64-Setup-0.1.4.exe'
...
`v0.1.4` 的预期签名状态是 `NotSigned`。还应确认 `release/win-unpacked` 中不存在 `grok.exe`，
...
带注释的 `v0.1.4` 标签和公开 prerelease，并且只把以下两个文件作为 GitHub Release 附件上传：

- `Grok-Desktop-Windows-x64-Setup-0.1.4.exe`
- `Grok-Desktop-Windows-x64-Setup-0.1.4.exe.sha256`
...
- `v0.1.4` 安装包未进行 Windows 代码签名，可能触发 SmartScreen；
- 主要能力、已知限制和校验值。
